...
func (f PermFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PermMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PermMutation", m)
}

// The PermGroupFunc type is an adapter to allow the use of ordinary
// function as PermGroup mutator.
type PermGroupFunc func(context.Context, *ent.PermGroupMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PermGroupFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PermGroupMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PermGroupMutation", m)
}
