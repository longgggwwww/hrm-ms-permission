...
func (rq *RoleQuery) QueryPerms() *PermQuery {
	query := (&PermClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithPerms(opts ...func(*PermQuery)) *RoleQuery {
	query := (&PermClient{config: rq.config}).Query()
...
func (rq *RoleQuery) loadPerms(ctx context.Context, query *PermQuery, nodes []*Role, init func(*Role), assign func(*Role, *Perm)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.PermsTable)
		s.Join(joinT).On(s.C(perm.FieldID), joinT.C(role.PermsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.PermsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.PermsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Perm](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "perms" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
