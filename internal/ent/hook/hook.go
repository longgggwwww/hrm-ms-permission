...
// The EventStoreFunc type is an adapter to allow the use of ordinary
// function as EventStore mutator.
type EventStoreFunc func(context.Context, *ent.EventStoreMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f EventStoreFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.EventStoreMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.EventStoreMutation", m)
}
